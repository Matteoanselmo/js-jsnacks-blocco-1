...
const randomGeneratorName = [];

for(let i = 0; i < 10; i++){
    let randomName = (nomi[Math.floor(Math.random() * nomi.length)].charAt(0).toUpperCase() + nomi[Math.floor(Math.random() * nomi.length)].substring(1)) + '\xa0' + (cognomi[Math.floor(Math.random() * cognomi.length)].charAt(0).toUpperCase() + cognomi[Math.floor(Math.random() * cognomi.length)].substring(1));
    randomGeneratorName.push(randomName);
}
console.log(randomGeneratorName);